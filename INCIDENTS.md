...
1. **Installed runtime evidence failures were opaque and repeated a large immutable image scan**
   - **Issue:** Real guarded tester and reviewer calls returned only `runtime_evidence_capture_failed`, masking the bounded capture stage that failed. Each call also performed two complete scans of roughly 60,000 immutable installed artifacts, and concurrent verifier calls repeated the same work. A controller `format_correction` call without its required report reached an unchecked cast and crashed while reading `scope`.
   - **Fix and why:** Added closed, non-sensitive evidence failure stages; shared and cached only successful verification of root-owned runtime-read-only provenance trees; retained a bounded Docker log scan with headroom for structured lines; and routed format correction through explicit schema checking plus the same assignment scope/evidence canonicalization used by report preflight and submission.
   - **Result:** Operators can diagnose the exact fail-closed evidence stage without exposing paths or exceptions, concurrent lanes reuse one verified immutable-image result, and malformed corrections return a stable error instead of throwing or bypassing assignment-bound evidence identity.

2. **Docker changes inspection timed out and guarded verifiers lacked the product toolchain**
   - **Issue:** The installed Gateway's Docker `/changes` response took about 8.4 seconds, but runtime evidence aborted every request after 5 seconds, so correctly authorized tester and reviewer calls failed at `container-inspection`. Their minimal Debian sandboxes also mounted only the read-only repository and contained no Node/Corepack/Yarn or prepared dependencies/browser runtime, so the named Playwright gate exited 127.
   - **Fix and why:** Gave only the Docker changes request a bounded 30-second timeout and single-flighted concurrent scans without caching results. Container inspection also rejects mounts that shadow installed runtime/provenance roots. Added an assignment-bound verifier image with pinned Yarn and browser libraries. An exclusive owner-only transaction, stored outside every Gateway bind including read-only binds, rejects unsafe state placement before `.env` or image mutation, builds an unpublished OCI candidate from the exact Gateway image ID, verifies its streamed manifests and toolchain offline, creates and reverifies a label-only final image with identical root filesystem layers, then atomically publishes its exact image ID and digests. Recovery journals tag intent before either build, resolves crash-created tags back to exact IDs, distinguishes durable commit from rollback, restores the byte-identical file and shell environment plus the exact prior Gateway image, and retains secured state on cleanup failure. The exact live protected candidate workspace remains read-only; Docker-in-Docker resolves the daemon-visible source from the Gateway's own mount, while dependency and browser subpaths use Docker 28's relative read-only `type=image` subpath contract. Runtime checks exact OCI identity, package/Yarn metadata and modes, Docker inspect's distinct image-mount identity fields, a strict Docker 28 no-network/no-log/init/resource profile, and protected assignment/candidate/workspace currency before and after execution. Container creation registers and validates the exact ID before first start; cancellable bounded pre/post checks use a one-use handle-local finalization authority; validation, execution, pruning, and cleanup never fall back to a mutable name or discard ownership after an unproven removal.
   - **Result:** Concurrent authorized evidence calls avoid repeated full artifact sweeps, and guarded verifier commands can use the immutable Yarn/Playwright toolchain while OCI substitution, wrong assignment/session/role/epoch, stale candidates, writable inputs, same-name replacement, and isolation-profile drift remain fail-closed.

3. **First publication, cancellation, and provisioning gaps weakened verifier lifecycle guarantees**
   - **Issue:** Before the verifier Compose overlay existed, setup checked transaction-state overlap only against published Gateway binds and could miss the declared verifier workspace. Post-execution validation inherited a caller cancellation signal and could be bypassed after the command stopped. Browser creation started an unregistered, unvalidated exact runtime and lacked one rollback boundary across port, bridge, and final registry publication. Legacy registry rows could also lend old age/image metadata to a newly observed exact runtime.
   - **Fix and why:** Setup now canonicalizes the prospective state root and declared verifier workspace independently before creating verifier state, permits only the platform-defined macOS `/var` alias, and then checks every published Gateway bind. Mandatory postchecks use a fresh bounded supervisor signal and preserve the command outcome when validation also fails. Browser creation publishes exact ownership before a closed pre-start identity check and rolls every later failure through exact runtime/bridge/registry cleanup; ambiguous cleanup retains exact ownership and fails closed. Runtime transitions are explicit, and every legacy-to-exact transition resets creation/image metadata after caller-side identity proof.
   - **Result:** First publication cannot place journals or backups in a verifier build context, caller cancellation cannot suppress the postcheck, failed browser provisioning does not silently orphan an unowned runtime, and prune age cannot be inherited from a different or legacy runtime.

4. **Verifier publication committed before the sandbox runtime contract was active**
   - **Issue:** Setup committed the verifier image and `.env` identity before requiring the Docker socket, validating the exact Gateway image's Docker CLI, writing the socket overlay, applying the sandbox policy, recreating the Gateway, and proving readiness. A later failure could therefore leave a published verifier identity paired with an unusable or partially configured runtime.
   - **Fix and why:** Docker socket and exact-image CLI checks now precede publication. The journal also protects the prior `.env`, sandbox config, overlay, and running or stopped Gateway state. Existing transaction recovery now runs before any setup environment upsert, build, or runtime mutation, so a substituted protected target cannot be replaced by ordinary setup work before recovery rejects it. The direct owner-only state root is pinned by device/inode as the setup process working directory before Compose inspection; its canonical path and pinned CWD are revalidated before recovery, environment mutation, and transaction creation, while every lock and transaction operation uses only relative paths from that CWD. Parent or state-root replacement therefore fails without following the substitute or using `/dev/fd`. The stable marker trust anchor is the already-existing direct canonical parent owned by the operator and not writable by group or other; this contract does not claim protection after removal of that anchor. Inside the state root, setup creates one cryptographically random, owner-only, single-link state-incarnation token with exclusive no-follow creation and durable file/directory sync. A restrictive, canonical, fsync-published active-state marker binds the parent and state device/inode, token digest, operation ID, and operation binding across `SIGKILL`; the journal binds the same token digest and operation. Cleanup first atomically and durably transitions that marker to an exact state-bound cleanup phase, and only that cleanup marker may be unlinked, so an unmatched active marker remains fail-closed. Device/inode is only a bounded locator: retry authenticates the configured state or an immediate displaced child only when its token also matches, rejects ambiguity, and never broad-scans. Every durable journal boundary and protected restore reasserts the original canonical path, inode, and token. A retry therefore rejects a replacement before setup mutation and recovers only after the operator restores the exact original state instance. Stale-state presence checks explicitly include dangling lock and transaction symlinks, which are rejected before protected setup mutation rather than being mistaken for absent paths. Every protected file is captured through a no-follow, single-link snapshot from a pinned parent directory; the journal records that parent identity, and recovery uses only relative leaf operations from the same pinned directory before an atomic byte-and-mode-exact replacement. The transaction directory itself is pinned by device/inode for each bounded helper by changing once into the verified direct directory; journal reads, temporary writes, fsyncs, publication renames, and exact cleanup then use only relative leaves while rechecking the requested path at publication boundaries. This avoids nonportable `/dev/fd/<directory-fd>/...` traversal. Strictly owned single abandoned journal candidates use deterministic bootstrap/update/restore names: a complete first journal is promoted, an incomplete first bootstrap is discarded, and a candidate beside an authoritative journal is reconciled; unknown, malformed, unsafe, or ambiguous candidates retain state. Each restore journals its exact temporary name, inode, original target inode, and durable phase before advancing; the temporary remains `0600` through write, digest verification, fsync, and rename, and the original final mode is applied only after the replacement phase is durable. Recovery deterministically reconciles a written temp, rename-before-journal, journal-before-mode, mode-before-journal, and mode-applied-before-clear boundary. Symlinks, hardlinks, special nodes, transaction/target parent replacement, or ambiguous temp/target identity retain recovery state without touching a substitute victim or deleting an untracked journal temporary. Verifier-image checks use zero-or-more positional arguments rather than empty array expansion so macOS Bash 3.2 remains compatible with nounset. Overlay publication, one-batch policy configuration, exact Gateway recreation, socket/config/health/readiness checks, and commit now share one recoverable transaction. Committed forward recovery retains ambiguous cleanup state, direct image consumers and dependent-image removal remain fail-closed, and the candidate parent image is retained while its metadata-only published child is installed.
   - **Result:** A verifier identity becomes durable only with its complete usable runtime contract. Within the stable owner-controlled marker parent, inode reuse alone cannot authenticate a replacement or redirect cleanup after abrupt death, and any pre-commit failure restores the exact prior setup state without leaving an untracked broad-mode copy of protected configuration behind.

## 2026-07-28

1. **Docker rebuild onboarding could not preserve an installed protected config**
   - **Issue:** `scripts/docker/setup.sh` built the requested image and then always ran onboarding plus Docker-default config writes. An installation that intentionally protected `openclaw.json` against replacement therefore failed with `EPERM` after the build, before guarded verifier publication could continue.
   - **Fix and why:** Added an explicit, default-off mode limited to guarded verifier publication. Before reading a config token or persisting setup state, the macOS-only path captures one canonical, no-follow, single-link config identity and digest, requires the same inode to carry `uchg` through the fixed `/usr/bin/stat` contract, separately validates exact local mode, runtime bind, and guarded sandbox policy from a new no-follow read, and skips ownership normalization, onboarding, and every setup config write. Token resolution and each later semantic or transaction check separately reopen the file without following links and revalidate identity, mode, parent, digest, and `uchg`; the flag is checked immediately before socket-equipped Gateway recreation, after the new exact container ID is durably journaled, at commit, and during recovery. A separate default-off authorization can preserve an explicitly supplied Docker runtime bind that differs from protected `gateway.bind`; it is valid only for a real read-only guarded mismatch, is persisted for recovery, and binds both bind values plus the authorization into the journal, operation, active marker, and create identity. This can preserve LAN exposure and is not treated as a more restrictive configuration. An explicit process-level return to normal config-writing mode suppresses and persists a stale inherited override as disabled, while an explicitly contradictory override still fails. Recovery separately authenticates genuine legacy v2 state, the exact b48 v3 pre-override marker/journal/create-binding shape, and current v4 bind-bound state; mixed, extended, downgraded, or altered shapes cannot cross those version boundaries. Before recreation, the journal durably records an authenticated create intent bound to the old Gateway ID, exact runtime image, Compose project/service identity, transaction format, config policy, and a transaction-specific candidate label derived from the active marker's cryptographically random transaction ID. The generated overlay applies that label before recreation. Recovery resolves that exact service, validates the exact candidate label, image, and Compose labels, and stops only that authenticated replacement before protected-config, `uchg`, or current live bind-policy validation; a revoked override or changed bind retains immutable state with the exact candidate stopped. Only a still-exact stopped candidate may be removed after validation, and removal failure also retains state. An exactly authenticated `committed` journal is cleanup-only: recovery revalidates its original protected policy and exact running Gateway, ignores newly changed live bind policy, clears owned transaction/lock/marker artifacts and temporary tags, and exits without rollback, Gateway restart, or republication. The authenticated journaled old image may also be removed only when the committed `gc-old-image=1` policy allows it and no consumer remains. A non-contradictory explicit return to normal mode atomically persists read-only and override values as disabled after committed tag/GC work but before transaction/marker/lock removal, fsyncs the final `.env`, and then fsyncs its repository parent before releasing recovery state. A pre-rename failure preserves the old environment; a file- or parent-fsync failure retains the committed transaction and lock even if `0/0` is already visible; and a plain retry repeats the barrier and completes cleanup without republication, rollback, or Gateway restart. Contenders cannot cross the lock before this fail-closed process-crash and `SIGKILL` handoff. These Node `fsync` calls provide best-effort filesystem persistence on macOS/Linux but not macOS `F_FULLFSYNC`; guaranteed sudden-power-loss durability is outside this bounded slice. An explicit simultaneous override enablement is rejected after authentication with committed state retained and no contradictory persistence. Missing, duplicate, altered, substituted, disappeared, reused-ID, or ambiguous identity retains state. The script never changes the flag, ACL, or protected file. The current journal and active marker cryptographically bind a versioned transaction format plus config policy and the transaction ID from which the candidate label is deterministically derived; damaged current records cannot downgrade to legacy handling, metadata-only state cannot impersonate a genuine legacy backup-bearing transaction, and genuinely legacy state-bound journals retain their original config-writing recovery. Read-only transactions retain metadata only and never plaintext config backups.
   - **Result:** Operators can rebuild and publish the guarded verifier runtime while preserving installed auth, model, channel, agent, and protected config bytes. Missing, malformed, linked, unreadable, policy-mismatched, replaced, drifted, unprotected, or downgrade-ambiguous state fails before acceptance or config replacement. The trusted operator must preserve `uchg`; removing it voids execution continuity and leaves the transaction fail-closed with any authenticated socket-equipped replacement stopped, analogous to replacing the owner-controlled verifier-state parent.

2. **Bare Docker image IDs were not runnable references**
   - **Issue:** The verifier transaction correctly captured exact `sha256:` image IDs, but passed those bare IDs to Dockerfile `FROM` and Compose `image`. Docker treated them as the repository name `library/sha256`, attempted a registry pull, and then repeated the same mistake while recovering a pre-create failure.
   - **Fix and why:** Keep the exact image ID as the journal, label, and post-create authority, but pin it to a deterministic content-addressed local alias for Docker's name-only build and Compose interfaces. The alias is verified against the exact ID immediately before every use. Before a verifier-enabled rebuild retags the normal runtime name, setup also pins the currently installed Gateway image for rollback. Recovery reuses an unchanged exact prior Gateway without recreating it; only a missing/replaced prior Gateway requires its separately pinned rollback alias.
   - **Result:** Local BuildKit and Compose receive valid local references without weakening exact-ID validation, a pre-create failure no longer restarts the untouched Gateway, and post-create rollback retains an exact runnable reference even after the ordinary runtime tag advances.

3. **Docker socket GID detection inspected the symlink instead of the socket**
