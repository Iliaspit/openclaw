    ...
    try {
      ...
      try {
        ...
        try {
          ...
          const childRouteTelemetrySessionKey =
            params.sessionKey &&
            (isSubagentSessionKey(params.sessionKey) || isAcpSessionKey(params.sessionKey))
              ? params.sessionKey
              : undefined;
          if (childRouteTelemetrySessionKey) {
            const estimateKnown = params.contextEngine?.info?.ownsCompaction !== true;
            const estimatedPromptTokens = estimateKnown
              ? preemptiveCompaction.estimatedPromptTokens
              : undefined;
            const headroomTokens =
              typeof estimatedPromptTokens === "number"
                ? contextTokenBudget - estimatedPromptTokens
                : undefined;
            const headroomPercent =
              typeof headroomTokens === "number"
                ? (headroomTokens / Math.max(1, contextTokenBudget)) * 100
                : undefined;
            const recordedHeadroom = await recordChildRouteContextHeadroomSnapshot({
              childSessionKey: childRouteTelemetrySessionKey,
              runId: params.runId,
              ...(typeof estimatedPromptTokens === "number" ? { estimatedPromptTokens } : {}),
              modelContextLimitTokens: contextTokenBudget,
              ...(typeof headroomTokens === "number" ? { headroomTokens } : {}),
              ...(typeof headroomPercent === "number" ? { headroomPercent } : {}),
              estimateSource: estimateKnown
                ? skipPromptSubmission
                  ? "preflight_estimate"
                  : "actual_request"
                : "unknown",
              lastCompactionStatus: preflightRecovery
                ? preflightRecovery.handled === true
                  ? "succeeded"
                  : "pending"
                : "none",
            });
            if (!recordedHeadroom.ok) {
              log.warn(
                `failed to record child route context-headroom telemetry: ${recordedHeadroom.error}`,
              );
            }
          }
